...
describe("convertFractions — nested constructs", () => {
    it("converts nested fraction: \\frac{\\frac{a}{b}}{c} → ((a/b)/c)", () => {
        expect(convertFractions("\\frac{\\frac{a}{b}}{c}")).toBe("((a/b)/c)");
    });

    it("converts fraction with root in numerator: \\frac{\\sqrt{a}}{b} → ((√a)/b)", () => {
        expect(convertFractions("\\frac{\\sqrt{a}}{b}")).toBe("((√a)/b)");
    });

    it("converts fraction with root in denominator: \\frac{a}{\\sqrt{b}} → (a/(√b))", () => {
        expect(convertFractions("\\frac{a}{\\sqrt{b}}")).toBe("(a/(√b))");
    });
});
